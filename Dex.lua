...
local function main()
	...
	ScriptViewer.Init = function()
		...
		-- ========== НОВЫЙ УЛУЧШЕННЫЙ ИНТЕРФЕЙС NOTEPAD ==========
		-- Верхняя панель кнопок (высота 28)
		local topBarHeight = 28
		local bottomBarHeight = 28
		
		codeFrame = Lib.CodeFrame.new()
		codeFrame.Frame.Position = UDim2.new(0,0,0, topBarHeight)
		codeFrame.Frame.Size = UDim2.new(1,0,1, -(topBarHeight + bottomBarHeight))
		codeFrame.Frame.Parent = window.GuiElems.Content
		
		-- Функция создания красивой кнопки
		local function makeBtn(text, color, pos, size)
			local btn = Instance.new("TextButton")
			btn.Parent = window.GuiElems.Content
		...
		-- ===== ВЕРХНИЙ РЯД =====
		-- 1. Copy
		local copy = makeBtn("📋 Copy", Color3.fromRGB(50, 50, 60), UDim2.new(0, 4, 0, 2), UDim2.new(0, 70, 0, 24))
		copy.MouseButton1Click:Connect(function()
			local ok, source = pcall(function() return codeFrame:GetText() end)
			if ok and env.setclipboard then
				env.setclipboard(source or "")
				print("[Dex++] Скопировано в буфер")
			end
		end)
		
		-- 2. Paste
		local paste = makeBtn("📄 Paste", Color3.fromRGB(50, 90, 140), UDim2.new(0, 78, 0, 2), UDim2.new(0, 70, 0, 24))
		paste.MouseButton1Click:Connect(function()
			local clip = nil
			
			-- Пробуем все возможные способы получить буфер
			local methods = {
				function() return getclipboard and getclipboard() end,
				function() return env.getclipboard and env.getclipboard() end,
				function() return syn and syn.getclipboard and syn.getclipboard() end,
				function() return Clipboard and Clipboard.get and Clipboard.get() end,
				function() return getgenv().getclipboard and getgenv().getclipboard() end,
				function() return crypt and crypt.getclipboard and crypt.getclipboard() end,
			}
			...
			if type(clip) == "string" and #clip > 0 then
				-- Заменяем весь текст (или можно было бы добавлять)
				pcall(function() codeFrame:SetText(clip) end)
				print("[Dex++] Текст вставлен из буфера (" .. #clip .. " символов)")
			else
				warn("[Dex++] Не удалось получить текст из буфера обмена")
			end
		end)
		
		-- 3. Save to Game (максимально жёсткая попытка записи)
		local save = makeBtn("💉 Save to Game", Color3.fromRGB(30, 140, 60), UDim2.new(0, 152, 0, 2), UDim2.new(0, 110, 0, 24))
		save.MouseButton1Click:Connect(function()
			local ok, source = pcall(function() return codeFrame:GetText() end)
			if not ok or type(source) ~= "string" or #source < 2 then
				warn("[Dex++] Текст пустой, нечего сохранять")
				return
			end

			print("[Dex++] Начинаю сохранение...")

			-- Способ 1: Пишем прямо в открытый скрипт
			if PreviousScr and typeof(PreviousScr) == "Instance" then
				print("[Dex++] Цель:", PreviousScr:GetFullName(), "Класс:", PreviousScr.ClassName)

				-- Пробуем обычный Source
				local success1, err1 = pcall(function()
					PreviousScr.Source = source
				end)

				if success1 then
					print("[Dex++] ✅ УСПЕХ: Source записан напрямую в", PreviousScr:GetFullName())
					return
				else
					warn("[Dex++] Способ 1 провален:", err1)
				end

				-- Способ 2: Через rawset / setreadonly обход
				local success2, err2 = pcall(function()
					local mt = getrawmetatable and getrawmetatable(PreviousScr)
					if mt then
						setreadonly(mt, false)
					end
					PreviousScr.Source = source
					if mt then setreadonly(mt, true) end
				end)

				if success2 then
					print("[Dex++] ✅ УСПЕХ: Source записан через rawset")
					return
				else
					warn("[Dex++] Способ 2 провален:", err2)
				end

				-- Способ 3: Удаляем старый скрипт и создаём новый с тем же именем
				local oldName = PreviousScr.Name
				local oldParent = PreviousScr.Parent
				local oldClass = PreviousScr.ClassName

				print("[Dex++] Пробую заменить скрипт полностью...")

				local success3, err3 = pcall(function()
					PreviousScr:Destroy()
					task.wait(0.05)

					local newScr = Instance.new(oldClass)
					newScr.Name = oldName
					newScr.Source = source
					newScr.Parent = oldParent
					PreviousScr = newScr
				end)

				if success3 then
					print("[Dex++] ✅ УСПЕХ: Скрипт полностью заменён →", oldName)
					return
				else
					warn("[Dex++] Способ 3 провален:", err3)
				end
			else
				warn("[Dex++] PreviousScr не найден. Открой скрипт через Explorer → View Script")
			end

			-- Способ 4: Создаём новый LocalScript в PlayerGui (последний шанс)
			print("[Dex++] Пробую создать новый LocalScript в PlayerGui...")
			local success4, err4 = pcall(function()
				local pg = game:GetService("Players").LocalPlayer:FindFirstChildOfClass("PlayerGui")
				if not pg then error("PlayerGui не найден") end

				local newScr = Instance.new("LocalScript")
				newScr.Name = "SavedScript_" .. math.random(1000,9999)
				newScr.Source = source
				newScr.Parent = pg
				print("[Dex++] ✅ Создан новый LocalScript в PlayerGui:", newScr.Name)
			end)

			if not success4 then
				warn("[Dex++] Все способы провалились. Последняя ошибка:", err4)
				warn("[Dex++] На этом executor запись Source заблокирована.")
			end
		end)

		
		-- 4. Del 1
		local delChar = makeBtn("⌫1", Color3.fromRGB(180, 50, 50), UDim2.new(0, 266, 0, 2), UDim2.new(0, 40, 0, 24))
		delChar.MouseButton1Click:Connect(function()
			local ok, txt = pcall(function() return codeFrame:GetText() end)
			if ok and type(txt) == "string" and #txt > 0 then
				pcall(function() codeFrame:SetText(txt:sub(1, -2)) end)
			end
		end)
		
		-- 5. Del 5
		local del5 = makeBtn("⌫5", Color3.fromRGB(150, 40, 40), UDim2.new(0, 310, 0, 2), UDim2.new(0, 40, 0, 24))
		del5.MouseButton1Click:Connect(function()
			local ok, txt = pcall(function() return codeFrame:GetText() end)
			if ok and type(txt) == "string" and #txt > 0 then
				pcall(function() codeFrame:SetText(txt:sub(1, math.max(0, #txt - 5))) end)
			end
		end)
		
		-- 6. Dump (если влезает)
		dumpbtn = makeBtn("Dump", Color3.fromRGB(70, 70, 80), UDim2.new(0, 354, 0, 2), UDim2.new(0, 50, 0, 24))
		...
		-- Execute сверху
		local executeTop = makeBtn("▶ Execute", Color3.fromRGB(40, 100, 180), UDim2.new(0, 408, 0, 2), UDim2.new(0, 75, 0, 24))
		executeTop.MouseButton1Click:Connect(function()
			local ok, source = pcall(function() return codeFrame:GetText() end)
			if ok and source and #source > 2 then
				local fn, err = loadstring(source)
				if fn then
					local success, res = pcall(fn)
					if success then
						print("[Dex++] Код выполнен успешно")
					else
						warn("[Dex++] Ошибка выполнения:", res)
					end
				else
					warn("[Dex++] Ошибка loadstring:", err)
				end
			else
				warn("[Dex++] Нечего выполнять")
			end
		end)
		
		-- ===== НИЖНИЙ РЯД =====
		execute = makeBtn("▶ Execute", Color3.fromRGB(40, 100, 180), UDim2.new(0, 4, 1, -26), UDim2.new(0.48, -6, 0, 24))
		execute.MouseButton1Click:Connect(function()
			local ok, source = pcall(function() return codeFrame:GetText() end)
			if ok and source and env.loadstring then
				local fn, err = env.loadstring(source)
				if fn then fn() else warn("[Dex++] Execute error:", err) end
			end
		end)
		
		clear = makeBtn("🗑 Clear", Color3.fromRGB(120, 40, 40), UDim2.new(0.52, 0, 1, -26), UDim2.new(0.48, -4, 0, 24))
		clear.MouseButton1Click:Connect(function()
			pcall(function() codeFrame:SetText("") end)
		end)
	...
	ScriptViewer.ViewScript = function(scr)
		...
		if not s or not source then
			...
			source = "-- Unable to view source (failed to get script bytecode)"
		else
			PreviousScr = scr
			dumpbtn.TextColor3 = Color3.new(1,1,1)

			-- Headers completely removed - pure source only
		end
...
-- HARD FIX v2: Полностью убираем выделение + улучшаем редактирование на Delta
task.spawn(function()
	local start = tick()
	while tick() - start < 20 do
		if rawget(getgenv(), "Lib") and type(Lib) == "table" and Lib.CodeFrame then
			local ok, err = pcall(function()
				local oldNew = Lib.CodeFrame.new
				if type(oldNew) == "function" then
					Lib.CodeFrame.new = function(...)
						local obj = oldNew(...)
						if obj then
							-- Полностью убиваем selection
							obj.SelectionRange = nil
							obj.SetCopyableSelection = function() end
							
							local oldRefresh = obj.Refresh
							...
							-- Пытаемся сделать курсор и ввод более отзывчивыми на мобильном
							if obj.Frame then
								obj.Frame.Active = true
								obj.Frame.Selectable = true
							end
						end
						return obj
					end
			...
			if ok then
				print("[Dex++ Fix] Hard no-selection + mobile edit patch applied")
				break
			end
		end
		task.wait(0.25)
